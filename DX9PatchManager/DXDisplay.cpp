...
void DisplayVoice(lpFM_BULK_OLD_PATCH lpOldPatch, int patchNo)
{
	...
	memcpy(dispName, lpOldPatch->PatchName, FM_PATCH_NAME_SIZE);
	ConsoleOut(L"\r\n\r\n>>>>>>>>>>>>>>>>>>>>>>>>>>>\r\n");
...
void DisplaySingleVoice(UCHAR *readBuffer)
{
	...
	char dispName[12];
	dispName[FM_PATCH_NAME_SIZE] = 0;
	memcpy(dispName, lpSingleOldPatch->PatchName, FM_PATCH_NAME_SIZE);
	...
	swprintf(tWork, BUFSIZE, TEXT("Name %S\r\n>>>>>>>>>>>>>>>>>>>>>>>>>>>\r\nAlgo:%d , Trans:%d KeySync:%d FB:%d\r\n"),
		dispName,
		lpSingleOldPatch->Algorithm.Algorithm,
		lpSingleOldPatch->Lfo.Transpose,
		lpSingleOldPatch->Algorithm.OscSync,
		lpSingleOldPatch->Algorithm.Feedback);
